...
use crate::api::Api;
use crate::java::{ArrayList, FromJava as _, ToJava as _};
use crate::{ctx, ffi};
...
pub(crate) unsafe extern "C" fn dispatch_command(
    ...
    let result = ffi::bridge(env, |env: &mut Env<'_>| -> eyre::Result<bool> {
        ...
        let args_vec = Vec::<String>::from_java(&mut Api::new(&mut *env), &args_arr)?;
...
pub(crate) unsafe extern "C" fn dispatch_tab_complete(
    ...
    let result = ffi::bridge(env, |env: &mut Env<'_>| -> eyre::Result<jobject> {
        ...
        // The sized inner frame bounds the per-completion string locals; the list itself is
        // returned into the caller's frame.
        let list = env.with_local_frame_returning_local::<_, JObject, eyre::Report>(
            completions.len() + 4,
            |env| -> eyre::Result<JObject<'_>> {
                let mut api = Api::new(env);
                let list: ArrayList<'_> = completions.to_java(&mut api)?;
                Ok(list.obj)
            },
        )?;
        Ok(list.into_raw())
